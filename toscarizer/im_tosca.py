...
def gen_tosca_yamls(app_name, dag, resources_file, deployments_file, phys_file, elastic,
    ...
    if os.path.exists(qos_contraints_file):
        with open(qos_contraints_file, 'r') as f:
            ...
            # Change the System key to system
...
def get_drift_detector(containers_file, oscar_cluster, qos_contraints, layer_num):
    ...
    if qos_contraints:
        ...
        raise Exception("No QoS constraints found for the Drift detector layer level (%s)." % layer_num)
        ...
        for res_id, res in compute_layer["Resources"].items():
            ...
            if res.get("operatingSystemImageId", "").startswith("aws"):
                if res.get("storageType", "") == "SSD":
                    wn["capabilities"]["host"]["properties"]["disk_type"] = "gp3"
